...
data_uri_regex = re.compile(r'data-uri\s*\(\s*([\'\"]?)([^\)\'\"]+)(\1)\s*\)')
...
class CssRewriter(object):
  ...
  def _found_resource(self, resource, uri_path):
    if uri_path.startswith('http://') or \
       ...
       uri_path.startswith('//') or \
       uri_path.startswith('data:'):
         return uri_path
  ...
  def fast_rewrite(self, code, resource):
    code = url_regex.sub(lambda m : 'url(' + self._found_resource(resource, m.group(2)) + ')', code)
    code = data_uri_regex.sub(lambda m: 'data-uri("' + self._am.get_abspath(self._found_resource(resource, m.group(2))) + '")', code)
    return code
