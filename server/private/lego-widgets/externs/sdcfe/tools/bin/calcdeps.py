...
include_regex = re.compile('goog\.include\s*\(\s*[\'\"]([^\)]+)[\'\"]\s*\)')
...
class ErTemplateParser(HTMLParser):
  def __init__(self):
  ...
  def handle_starttag(self, tag, attrs):
    ...
    if attr_dict.get('ui') is not None:
      value = attr_dict.get('ui')
      for option in value.split(';'):
        pairs = option.split(':')
        if len(pairs) == 2 and pairs[0] == 'type':
          ui_type = pairs[1]
          if ui_type.find(".") == -1:
            self._uis.add("ui." + ui_type)
          else:
            self._uis.add(ui_type)
          break

  def get_deps(self):
    return list(self._uis)

def GetDepsFromTemplate(filename):
  # logging.error(filename)
  parser = ErTemplateParser()
  parser.feed(open(filename).read())
  return parser.get_deps()

def BuildDependenciesFromFiles(files, base_path):
    ...
    try:
      ...
      for line in file_handle:
        ...
        if re.match(include_regex, line):
          ui_deps_file = re.search(include_regex, line).group(1)
          if ui_deps_file.endswith(".html") or \
              ui_deps_file.endswith(".htm"):
            ui_deps = GetDepsFromTemplate(os.path.join(os.path.dirname(base_path), ui_deps_file))
            for deps in ui_deps:
              requires.add(deps)
        if re.match(prov_regex, line):
          provides.add(re.search(prov_regex, line).group(1))
      dep.requires.extend(list(requires))
      dep.provides.extend(list(provides))
    finally:
      file_handle.close()
    result.append(dep)
...
def CalculateDependencies(paths, inputs):
  ...
  Returns:
    ...
    inputs.
  """
  base_path = FindClosureBasePath(paths)
  deps = BuildDependenciesFromFiles(paths + inputs, base_path)
...
def GetDepsLine(dep, base_path):
  ...
  return 'goog.addDependency("%s", %s, %s);' % (
      GetRelpath(dep.filename, base_path), provides, requires)
...
def PrintDeps(source_paths, deps, out):
  ...
  deps = BuildDependenciesFromFiles(source_paths + deps, base_path)
  deps.sort(lambda a, b: cmp(a.filename, b.filename))
  for dep in deps:
...
def IsSupport32BitJVM():
  args = ['java', '-d32', '-version']
...
def Compile(compiler_jar_path, source_paths, out, flags=None):
  ...
  if IsSupport32BitJVM():
    args += ['-d32']

  args += ['-client', '-jar', compiler_jar_path]
  ...
  logging.warning('Compiling ' + str(len(source_paths)) + ' js files with gcc.jar')
  logging.debug('Compiling with the following command: %s', ' '.join(args))
  proc = subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
  (stdoutdata, stderrdata) = proc.communicate()
  if proc.returncode != 0:
    ...
    if stderrdata:
      logging.warning(stderrdata.strip().splitlines().pop())
