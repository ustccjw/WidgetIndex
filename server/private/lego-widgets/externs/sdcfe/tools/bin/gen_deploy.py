...
# $Id: gen_deploy.py 15220 2012-12-04 13:05:55Z liyubei $ 
...
def gen_deploy(options, args):
  ...
  # 是否是部署ad style
  deploy_style = options.deploy_style
  ...
  try:
    ...
    if all_in_one:
      if deploy_style:
        ep_js_fh.write("(function(AD_CONFIG){\n")
      else:
      ...
      compiled_pattern = re.compile(r'(\.compiled-[a-f0-9]{8}\.js)$');
      # 输出third_party_files
      for f in third_party_files:
        logging.debug(f)
        # 忽略tangram，最后用Ftangram把缺失的函数补充完毕即可.
        if f.find("tangram") == -1:
          if compiled_pattern.search(f):
            # 忽略*.compiled.js，因为都是处理过的代码了，直接合并即可
            zip_code = open(_(f)).read()
          else:
      ...
      if options.ad_style_wrapper == 'None':
        # 知心模板形式的开发，需要把css单独输出来制作smarty的模板
        ep_css_fh = open(_(ep_css_file), 'wb')
        ep_css_fh.write(zip_css_string(ep_css_code_merged))
        ep_css_fh.close()
      else:
      ...
      if not options.disable_ftangram:
        ...
        tangram_code = zip_js_string(tangram)
        ep_js_fh.write(tangram_code.replace('var T,baidu=T=baidu||', 'var baidu='))
        ep_js_fh.write(";\n");
  ...
  ep_signed_css_file = "about:blank" if all_in_one and options.ad_style_wrapper != 'None' else am.file_sign(_(ep_css_file))
  app_cfg = {
    "app.js.path" : ep_signed_js_file,
    "app.css.path" : ep_signed_css_file
  }
  if all_in_one:
    if options.output_file_template and os.path.exists(options.output_file_template):
      # 采用输出模板来确定index.html的内容
      output_tpl = open(options.output_file_template, 'r').read()
      output_content = output_tpl.replace('%(app.js.path)s',
          ep_signed_js_file).replace('%(app.css.path)s', ep_signed_css_file)
      write_file(_(ep_top_file), output_content, True)
    else:
