 ...
 * $Id: server.js 14970 2012-11-26 12:20:22Z liyubei $
 *
 **************************************************************************/
...
function rewriteResourcePath(code, absPath, reqPath) {
  ...
  var rewritedCode = code.replace(urlPattern, function(match, p0, p1, p2, p3, offset, string) {
    ...
    if (p0 === 'url') {
      ...
      // src/../../../a.gif
      var resource = path.relative('.', path.normalize(path.join(dirName, p2)));
      var url = resource.replace(/\\/g, '/');
    }
    return p0 + '(' + p1 + url + p3 + ')';
  });
    ...
    fs.exists(abs_path, function(exists) {
      ...
      if (/\.less$/.test(abs_path)) {
        ...
        fs.readFile(abs_path, function(err, input) {
          ...
          var options = {
                'paths' : [path.dirname(abs_path), server.getAbsPath('src/css/less')],
                'filename' : abs_path
              },
...
if (!fs.existsSync(server.getAbsPath('src/css/less'))) {
