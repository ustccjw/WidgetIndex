 ...
 * $Id: server.js 156521 2012-12-28 06:35:18Z  $ 
 * 
 **************************************************************************/
...
/**
 * 合并多个Buffer
 * @param {Array.<Buffer>} buffers 要合并的buffer数组
 * @return {Buffer} 合并之后的buffer
 */
function joinBuffers(buffers){
  if(!buffers || buffers.length <= 0)return new Buffer(0);
  // calculate the total length of all Buffers
  var total = 0;
  for(var i = 0; i < buffers.length; i++){
    total += buffers[i].length;
  }
  // copy all Buffers to a new Buffer
  var result = new Buffer(total),
      curIndex = 0;
  for(var i = 0; i < buffers.length; i++){
    buffers[i].copy(result, curIndex);
...
function proxyRequest(request, response, chunks) {
  ...
  if (!host) {
    ...
    if (host[1] === 80) {
      request.headers['host'] = host[0];
    } else {
      request.headers['host'] = host[0] + ':' + host[1];
    }
    ...
    proxy_request.addListener('response', function (proxy_response) {
      ...
      proxy_response.addListener('end', function() {
        var response_body = joinBuffers(response_chunks);
    ...
    // directly write Buffer
    proxy_request.write(joinBuffers(chunks));
    proxy_request.end();
  }
}
...
function ErServer() {
  this._handlers = {};
  this.file = new(static.Server)(argv.root, {cache: 60 * 60 * 24 * 365});
}
  ...
  http.createServer(function(request, response){
    ...
    request.addListener('end', function (){
      ...
      if (handler = me._handlers[decodeURI(parsed_url.pathname)]) {
        // 存在自定义的处理函数.
        try {
          var rv = handler(request, response, joinBuffers(chunks));
          if (typeof rv === 'string') {
        ...
        fs.stat(pathname, function(error, stats){
          if (!error) {
            ...
            } else if (/\/src\/.*\.js\?amd=1$/.test(request.url) ) {
              var xutil = require('../../../build/task/util');
              var code = xutil.convertToAMDModule(pathname);
              response.writeHead(200, {
