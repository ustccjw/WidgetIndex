package baidu.dv.graphs {
	...
	import baidu.dv.graphs.BarItem3D;
	 ...
	 */
	public class BarGraph3D extends GridBasedGraph {
		...
		protected function genItems():void {
			...
			if (_itemRenderer || _seriesRenderers.length > 0) {
				for (var i:Number = 0, len:Number = _data.length * numSeries; i < len; i++) {
					...
					//自身数据
					var selfValue:Number = 0;
					if(_numSeries > 1) {
						selfValue = _data[Math.floor(i / numSeries)][seriesIndex];
					}else {
						selfValue = _data[i];
					}
					
					//透视数据
					if(item is BarItem3D) {
						(item as BarItem3D).allMinValue = _allMixValue;
						(item as BarItem3D).allMaxValue = _allMaxValue;
						(item as BarItem3D).selfValue = selfValue;
						(item as BarItem3D).dataLength = _data.length;
						(item as BarItem3D).numSeries = _numSeries;
					}
		...
		protected function caculatePerspectiveData():void {
			...
			if(_data && (_data is Array) && (_data.length > 0)) {
				for(var i:int = 0, iLen:int = _data.length; i < iLen; i++) {
					var d:* = _data[i];
					if(d is Array) {
						for(var j:int = 0, jLen:int = d.length; j < jLen; j++) {
							var dj:Number = Number(d[j]);
							_allMaxValue = Math.max(_allMaxValue, dj);
							_allMixValue = Math.min(_allMixValue, dj);
						}
					}else {
						d = Number(d);
						_allMaxValue = Math.max(_allMaxValue, d);
						_allMixValue = Math.min(_allMixValue, d);
					}
				}
			}
		}
