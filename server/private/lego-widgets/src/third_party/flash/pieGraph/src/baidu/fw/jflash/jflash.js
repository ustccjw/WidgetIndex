...
SWFObject = function(json){
	//检查参数合法性
	if(json.cid==null||json.swfId==null||json.movie==null){
		alert("SWFObject:参数不够");
	}
	
	//默认值
	this.cfg = {
		cid: null,//容器的id
		swfId: null,//object 或者 embed 标签的id
		width: "100",
		height: "100",
		align: "middle",
		movie: null,//swf文件的 url
		allowScriptAccess:"always",
		salign: "lt",
		wmode: "window",/*transparent,window,opaque*/
		scale: "noscale",
		flashVars: ""
	};
	
	//初始化
	this.cfg = Utils.mo(json,this.cfg);
...
Utils = {
	...
	f: function(json){
		var embedTpl = '<embed id="#{swfId}" name="#{swfId}" src="#{movie}" flashVars="#{flashVars}" width="#{width}" height="#{height}" align="#{align}" type="application/x-shockwave-flash" pluginspage="http://www.adobe.com/go/getflashplayer" wmode="#{wmode}" scale="#{scale}" salign="#{salign}" />';
		var objectTpl = '<object id="#{swfId}" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="#{width}" height="#{height}" align="#{align}"><param name="movie" value="#{movie}" /><param name="flashVars" value="#{flashVars}" /><param name="wmode" value="#{wmode}" /><param name="scale" value="#{scale}" /><param name="salign" value="#{salign}" /></object>';
