package
{
  ...
  public class ToolTips extends Sprite
  {
	  ...
	  public function setContent(arr:Array,n:int,_x:Number,_y:Number):void
	  {
		  ...
		  if (s4 == "-1" || s4 == "-1.00") textField.text = s0 + "\r点击：" + s1 + "\r展现量：" + s3 + "\r点击率：" + Math.round(Number(s2) * 10000) / 10000 + "%";
		  else textField.text = s0 + "\r消费：￥" + Number(s4).toFixed(2) +"\r点击：" + s1 + "\r展现量：" + s3 + "\r点击率：" + Math.round(Number(s2)*10000)/10000 + "%";
		  textField.alpha = 0.7;
		 ...
		 // trace(container.numChildren+"是孩子个数")
	  }
  }
}