package baidu.lib.utils {
	 ...
	 * 以上是原始作者的注释
	 * 
	 * 这个类是二进制数据上传辅助类，可以同时上传多个二进制数据，同时携带其他的key-value参数对
	 * 
	...
	public class UploadPostHelper {
		 ...
		 * 
		 * @param	fileByteArrayList	二进制数据列表，每个元素为一个对象，包含3个属性{fileName, byteArray, fieldName}
		 * 
		 * fileName:String, byteArray:ByteArray, uploadDataFieldName:String = "filedata"
		 */
		public static function getPostData(fileByteArrayList:Array, parameters:Object = null):ByteArray {
			...
			for (var j = 0, jLen:int = fileByteArrayList.length; j < jLen; j++ ) {
				var file:Object = fileByteArrayList[j];
				var fileName:String = file["fileName"];
				var byteArray:ByteArray = file["byteArray"];
				var uploadDataFieldName:String = file["fieldName"];
		 ...
		 * 
		 * @param	fileName	文件名
		 * @param	byteArray	文件内容数据流
		 * @param	fieldName	字段名
		 * 
		 * 这个方法是原始方法，只上传一个二进制数据的时候可以使用
		 */
		public static function getPostDataX(fileName:String, byteArray:ByteArray, uploadDataFieldName:String = "filedata", parameters:Object = null):ByteArray {
