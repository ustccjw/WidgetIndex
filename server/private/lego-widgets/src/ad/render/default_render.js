 ...
 * $Id: default_render.js 11342 2012-08-28 03:37:54Z liyubei $
 *
 **************************************************************************/
...
/** @override */
ad.render.DefaultRender.prototype.attachToElements = function(widgets, wrapper) {
    for (var i = 0, rows = widgets; i < rows.length; i++) {
        var rowRoot = wrapper.children[i];
        if (rowRoot) {
            for (var j = 0, cols = rows[i], colRoot; j < cols.length; j++) {
                colRoot = rowRoot.children[j];
                if (colRoot) {
                    cols[j].setId(colRoot.id);
                }
                else {
