...
ad.service.StatisticsService.prototype.log = function(opt){
    var logUrl = RT_CONFIG.HOST('wbapi.baidu.com') + '/v.gif?plid=' + this.id;
    var params = {
        'q': window['bdQuery'],
        't': +new Date()
    };
...
ad.service.StatisticsService.prototype.init = function(id){
    ...
    var scrollHandler = function(){
        ...
        if(height < criticalHeight){
            //被遮住的超过50%
            if(_scrollTimer){
                ad.base.clearTimeout(_scrollTimer);
                _scrollTimer = null;
            }
        } else {
            if(!_scrollTimer){
                //start timer
                _scrollTimer = ad.base.setTimeout(function(){
                    me.log({
                        'title': 'view'
                    });
    ...
    baidu.on(maDom, 'mouseenter', enterHandler);

    var leaveHandler = function(){
        sendHangTimeLog('out');
    };
    baidu.on(maDom, 'mouseleave', leaveHandler);
    ...
    baidu.on(maDom, 'click', clickHandler);

    ad.base.registerUnloadHandler(function() {
        baidu.un(document, 'scroll', scrollHandler);
        baidu.un(window, 'resize', scrollHandler);
        baidu.un(maDom, 'mouseenter', enterHandler);
        baidu.un(maDom, 'mouseleave', leaveHandler);
        baidu.un(maDom, 'click', clickHandler);
    });
};
