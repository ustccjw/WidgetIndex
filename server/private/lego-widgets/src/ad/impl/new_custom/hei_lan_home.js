...
/**
 * src/ad/impl/new_custom/hei_lan_home.js ~ 2014/08/27 15:37:16
...
goog.include('ad/impl/new_custom/hei_lan_home.less');

goog.provide('ad.impl.new_custom.HeiLanHome');
...
ad.Debug(function(async) {
    ...
    ad.widget.TabContainer.prototype.calcTabHeadWidth = function(tabCount) {
        ...
        var itemWidth = Math.floor(totalWidth / tabCount);
            ...
            if (select) {
                select.addListener(ui.events.CHANGE, function(city, shop, depth) {
                    ...
                    if (detail) {
                        ...
                        detail['img_src'] = RT_CONFIG.HOST('ecma.bdimg.com') 
                            + '/adtest/e95d4b929e1cb6938751645176a62178.jpg';
    ...
    var imgs = new ad.widget.ImageNormal(AD_CONFIG['products']);
    ...
    material.setWidgets(
        ...
        [ getTabContainer() ],
        [ new ad.widget.ButtonGroup(AD_CONFIG['bottom_button']) ],
        [ fwc ]
    );
    if (async === true) {
        return material;
    ...
    leftVideo.addListener(ui.events.VIDEO_CLICK, function() {
        leftVideo.pause();
        showFWC(0);
        leftVideo.sendLog('float window open');
        return false;
    });
