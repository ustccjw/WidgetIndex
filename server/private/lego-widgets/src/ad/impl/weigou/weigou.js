...
goog.require('ad.widget.weigou.DetailView');
goog.require('ad.widget.weigou.ListView');
...
goog.include('ad/impl/weigou/weigou.less');

goog.provide('ad.impl.weigou.Weigou');
...
ad.Debug(function() {
    ...
    function gotoDetail(id) {
        ...
        ad.impl.weigou.dal.detailRecommend(id, reg, function(data) {
            ...
            if (data['recommends'] && data['recommends'].length !== 0) {
                rcmdData = data['recommends'];
            }
            var detailView = new ad.widget.weigou.DetailView(data['result'], rcmdData);
    ...
    var view;
    if (data['isDetail']) {
        view = new ad.widget.weigou.DetailView(data['data']);
        view.addListener(ad.impl.weigou.events.PURCHASE_VIEW, gotoPurchase);
    } else {
        view = new ad.widget.weigou.ListView(data);
    ...
    if (ad.impl.weigou.dist.PC && !ad.impl.weigou.dist.IPAD) {
        // 检测有多少人安装了adblock，如果没有这个全局变量，则发送一条统计
        if (!window['ECOM_WEIGOU_AD_DETECTOR']) {
            log.sendLog({
                'item': 'ad-blocked'
            });
        }
    ...
    if (ad.impl.weigou.dist.IPAD) {
        baidu.g("ecl-weigou-back-btn").innerHTML = '返回';
    }
