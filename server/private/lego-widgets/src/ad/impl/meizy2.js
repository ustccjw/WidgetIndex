 ...
 * $Id: meizy2.js 150523 2012-10-30 10:06:05Z  $
 *
 **************************************************************************/
...
/**
 * src/ad/impl/meizy2.js ~ 2013/07/10 14:25:38
...
goog.include('ad/impl/meizy2.less');

goog.provide('ad.impl.Meizy2');
...
ad.Debug(function(async){
    ...
    //创建第二帧内容
    function createDesc() {
        imgDesc = baidu.dom.create('div', {'class': 'ec-img-desc'});
        imgDesc.innerHTML = ['<div class="ec-share-back" id="' + imgCartoon.getId("share-back") + '"></div>',
            '<div class="ec-share-pic">',
                '<a id="' + imgCartoon.getId("share-pic-wrap") + '" target="_blank" title2="说明图片" href="">',
                '</a>',
            '</div>',
            '<div class="ec-share-desc">',
                '<a id="' + imgCartoon.getId("share-desc-wrap") + '" target="_blank" title2="说明描述" href="">',
                '</a>',
            '</div>',
            '<div class="ec-share-link">',
                '<a rel="nofollow" id="' + imgCartoon.getId("sina-share") + '" class="ec-sina-share" title="新浪微博" title2="新浪微博">新浪微博</a>',
                '<a rel="nofollow" id="' + imgCartoon.getId("qqweibo-share") + '" class="ec-qqweibo-share" title="腾讯微博" title2="腾讯微博">腾讯微博</a>',
                '<a rel="nofollow" id="' + imgCartoon.getId("renren-share") + '" class="ec-renren-share" title="人人网" title2="人人网">人人网</a>',
            '</div>'].join('');
        ...
        baidu.on(imgCartoon.getId("qqweibo-share"), 'click', function() {
            ...
            var url = 'http://share.v.t.qq.com/index.php?c=share&a=index&url=' + 
                      ...
                      '&title=' + encodeURIComponent(data['share_text']) + 
                      '&pic=' + encodeURIComponent(data['share_pic']) +
                      '&site=' + encodeURIComponent(data['share_site']);
            var params = getParamsOfShareWindow(607, 523);
            window.open(url, 'share', params);
        });
        baidu.on(imgCartoon.getId('renren-share'), 'click', function(){
            ...
            var url = 'http://widget.renren.com/dialog/share?resourceUrl=' + 
                      ...
                      '&title=' + encodeURIComponent(data['share_text']) + 
                      '&pic=' + encodeURIComponent(data['share_pic']) +
                      '&srcUrl=' + encodeURIComponent(data['share_site']) + 
                      '&description=';
            var params = getParamsOfShareWindow(607, 523);
            window.open(url, 'share', params);
        });
