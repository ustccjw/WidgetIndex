...
goog.provide('ad.impl.ishop.util');
...
ad.impl.ishop.util = {
    ...
    subByte: function(source, length, tail){
        ...
        tail = tail || '';
        if (length < 0 || ad.impl.ishop.util.getByteLength(source) <= length) {
    ...
    ellipsis: function(source, length){
        return ad.impl.ishop.util.subByte(source, length, '...');
    },
...
ad.impl.ishop.util.storage = (function(){
    ...
    function _createUserData() {
        ...
        div.id = _guid + '-storage';
        div.style.display = 'none';
        document.body.appendChild(div);
        // 下面这种方式有问题，format与我们的不兼容
        /*
...
//            size: 4 * 1024,
            set: function(key, value, callback, options) {
                ...
                callback && callback.call(this, _status.SUCCESS, value);
            },

            get: function(key, callback) {
                var val = baidu.cookie.get(_getKey(key));
                callback && callback.call(this, _status[val ? 'SUCCESS' : 'FAILURE'], val);
            },
            del: function(key, callback) {
