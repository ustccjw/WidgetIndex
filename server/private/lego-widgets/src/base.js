...
/**
 * @return {boolean} true in fe-debug mode, otherwise return false.
 */
goog.isDebugMode = function() {
  if (!COMPILED) {
    return (/ed=|enable_debug=|\.app\.html/.test(document.location.href));
  } else {
    return false;
  }
}
...
if (!COMPILED) {
  ...
  //==============================================================================
  // Web Application Loader Enhancements (by leeight)
  //==============================================================================
...
goog.include = function(path) {
  if (!COMPILED) {
    ...
    if (/\.css$/.test(path) || /\.less$/.test(path)) {
      if(!(absPath in goog.asyncPathMap)){
        goog.asyncStyles.push(absPath);
        goog.asyncPathMap[absPath] = true;
      }
