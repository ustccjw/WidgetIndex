...
er.Action.prototype.enter = function(argMap) {
    ...
    var pdc = er.Pdc.getInstance();
    pdc.reset();
    pdc.setPage(er.locator.getPath());
...
er.Action.prototype.done = function() {
    ...
    if (er.controller.permit) {
        for (var i = 0, j = all.length; i < j; i++) {
            ...
            if (auth) {
                var hasAuth = er.controller.permit(auth);
                if (!hasAuth) {
...
er.Action.prototype.back = function() {
    ...
    if (!referer || this.USE_BACK_LOCATION) {
        ...
        referer && er.locator.redirect(referer);
    }else {
 ...
 * @protected
 */
er.Action.prototype.onbeforeleave = baidu.fn.blank;
 ...
 */
er.FormAction.prototype.onSubmitFail = function(opt_data) {
    if (this.btnSubmit) {
        this.btnSubmit.enable();
    }
};
